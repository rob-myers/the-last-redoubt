...
- If $dp_A > 0$ then $A$ is moving away from $B$, in which the NPC's don't collide.<QedIcon/> <SideNote>Technically they may be colliding, but we allow them to separate.</SideNote>
...
- If $x > 0$ there are two solutions.
  ...
  Finally, if $0 < t_0 \leq t_\Omega$ then the NPCs collide.<QedIcon/>
