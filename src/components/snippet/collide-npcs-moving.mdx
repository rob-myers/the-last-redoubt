...
- If $x > 0$ there are two solutions.
  By the above assumptions, $dp_A < 0$ and $dp_B > 0$ and $c > 0$.
  Hence $0 < x \leq b^2$, so that $-b - \sqrt{x} > 0$.
  Then _both_ real-valued solutions are positive and $t_0 := (-b - \sqrt{x}) . \frac{1}{a}$ is the smaller one.
  If $0 < t_0 \leq t_\Omega$ then the NPCs collide, otherwise they don't.<QedIcon/> <SideNote>Technically they do not collide whilst both NPCs are moving.</SideNote>
