...
For example, Super Mario 3 consists of mostly independent individual NPC behaviours running in parallel.
...
For example, the lighting above is achieved via two PNGs (a light and dark one) and a [CSS clip-path](https://developer.mozilla.org/en-US/docs/Web/CSS/clip-path).
We'll play the actual game in the next subsection.
For those itching to see how it is built, [check out this CodeSandbox](__TODO__).

### How to direct

Importantly, the videos above show a [terminal](https://xtermjs.org/) (a black window with mostly green text).
...
1. Try clicking _interact_ below.
   ...
   Then clicking disable <Icon icon="cross-circle" small inline />,
...
3. Finally click the 1st Tab again...
