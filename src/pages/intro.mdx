...
3. Finally return to the 1st tab and scroll to the top.
  ...
  <aside>
    ...
    Our [shell](https://en.wikipedia.org/wiki/Shell_(computing)#Command-line_shells) wraps JavaScript (JS) in a Command Line Interface.
    JS is concerned with **values** (primitive or object) rather than files.
    Each terminal has an associated root object where
    e.g. `root.home.PROFILE` is a string defining the initialisation script.
    Shells usually don't echo the initialisation script as it runs.
    Ours pastes it into the terminal, producing the behaviour above.
