...
We need a Story and Characters which inhabit a World.
Concerning the latter, thankfully we've already done the groundwork.
...
We now turn our attention to the [terminal](https://en.wikipedia.org/wiki/Xterm),
the black pane with mostly green text.
When prompted by `$` we may type commands into it and press _enter_ to run them.
What does this mean?
Well, to _run_ a command we (a) [parse it](https://en.wikipedia.org/wiki/Parsing#Computer_languages), and (b) [interpret it](https://en.wikipedia.org/wiki/List_of_command-line_interpreters) e.g. invoke functions previously defined by the system or user.
