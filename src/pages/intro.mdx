...
To create a game we must fabricate a _world_ i.e. something we can see and move around in.
...
But how should we fabricate this pyramid?
How should we build The Last Redoubt?

> Thankfully we've done the groundwork, as shown in the videos below.

<iframe id="intro-first-videos" className="youtube" width="100%" height="464" src="https://www.youtube.com/embed/videoseries?list=PLTcU-Qpr40X6hq3GSbY8K92DR_DUSgzim" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
We now summarise some of what we've just seen.
The game has a birdseye viewpoint (truly top-down).
...
Let's [refer back](#intro--tech-stack "@anchor") to our standard web dev approach.
Consider the lighting i.e. the lit part of the rooms, which changes as the Player opens/closes/enters doors.
We achieve it using two copies of the [same PNG](/geomorph/g-301--bridge.png "@new-tab"), one above the other.
...
We'll interact with the actual world in the next section.
For those itching to see how it is built, check out the CodeSandbox [here](https://codesandbox.io/s/tty-world-1-0s9wfl?file=/src/index.js "@new-tab").

### Directing the world

Let's not lose sight of our motivation: **understanding and improving Game AI**.
Rather than relying on the Player interacting with mostly independent parallel systems,
we'll intentionally modify/monitor the Player's experience.
Although a persistent world must exist from the Player's perspective,
it is the latter we care about, not the former.
We've made good progress fabricating a world, but how to shape/control/direct it?

The [videos above](#intro-first-videos "@anchor") show a [_terminal_](https://xtermjs.org/) i.e. the black window with mostly green text.
...
If Game AI (NPC behaviour) is the interaction between the Director (a human-software amalgam) and the Player,
then the Director _directs_ by running commands in one or more terminals,
...
3. Finally return to the 1st tab and scroll to the top.
  ...
  | Line | Info |
  | --- | ----------- |
  | <InlineUniCode>ℹ️</InlineUniCode>`  Running /home/PROFILE` | The absolute path <InlineCode>/home/PROFILE</InlineCode> refers to a string i.e. the terminal's initialisation script. Try typing it into the terminal, or click and paste (easier on a mobile). |
  | `source /etc/util-1` | The 1st line of the script at `/home/PROFILE`. This command executes the script at <InlineCode>/etc/util-1</InlineCode>, defining various util functions. |
  | `source /etc/game-1` | From `/home/PROFILE`, <InlineCode>/etc/util-1</InlineCode> defines game-related functions. List all functions in the console via <InlineCode>declare \| log</InlineCode>. |
  | `await-world` | From `/home/PROFILE`, this command awaits a world with worldKey <InlineCode>/home/WORLD_KEY</InlineCode>. While polling it sends lines, as we saw. |
...
Let us finish this section with some videos illustrating the above example.
...
__TODO__
- Story provides some guidance
- Teleglitch provides some guidance
- ...
