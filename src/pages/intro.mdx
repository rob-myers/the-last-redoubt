...
> and JavaScript-driven rendering or simulation (physics).
...
| `source /etc/game-1` | From `/home/PROFILE`, <InlineCode>/etc/util-1</InlineCode> defines game-related functions. List all functions via <InlineCode>declare</InlineCode>. Those with a dev console can <InlineCode>declare \| log</InlineCode>. |
...
(b) believable NPC behaviour is analogous to the Imitation Game.
...
So what's our chosen game going to be?
...
| Story Overview | Andros must help defend the pyramid against invaders: _mental effluents_ and _unnatural incarnations_ who've broken through the Air Clog. |
| Story Influences | [The Night Land](https://en.wikipedia.org/wiki/The_Night_Land) (1912), [Theravada Buddhism](https://en.wikipedia.org/wiki/Theravada).  |
| Gameplay Influences | <table style="font-size:0.95rem; padding: 8px;"><tr><td> [Teleglitch](https://en.wikipedia.org/wiki/Teleglitch "@new-tab") </td><td> Top-down survival-horror roguelike </td></tr> <tr><td> [Fallout 1, 2](https://en.wikipedia.org/wiki/Fallout_(video_game) "@new-tab") </td><td> Oblique turn-based RPGs </td></tr> <tr><td> [GTA 1, 2](https://en.wikipedia.org/wiki/Grand_Theft_Auto_(video_game) "@new-tab") </td><td> Top-down open-world action games. </td></tr></table> |

<aside title="game-influences">

Teleglitch (2013) is the strongest gameplay influence.
The protagonist must escape a military base (every NPC is hostile).
The top-down line-of-sight view plus other factors produce a feeling of dread.
...
Then a level/scenario should be kept in mind whenever we develop Game AI (each level being built from many scenarios).

__TODO__ outline first level
