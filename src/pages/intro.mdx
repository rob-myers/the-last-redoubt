...
We're talking of course about [_video games_](https://en.wikipedia.org/wiki/Video_game)
where the acronym _AI_ stands for Artificial Intelligence. 
Roughly speaking, _Game AI_ amounts to a loose collection of algorithms and data structures for controlling [NPCs](https://en.wikipedia.org/wiki/Non-player_character).
These Non-Player Characters must behave realistically,
steadily drawing the Player into the fabrication.

Reading Game AI tutorials, one gets the impression that intelligent behaviour just _emerges_ from the right ingredients.
...
To make a game, we need narratives involving characters inhabiting a world.
...
Let us name two characters i.e. Andros (the protagonist) and Cartesius (his Preceptor);
...
Our overall narrative is familiar: the protagonist defends the inhabitants of the pyramid from invading evil forces.
Then we need to build this pyramid (The Last Redoubt).
...
We now turn our attention to the [terminal](https://xtermjs.org/),
