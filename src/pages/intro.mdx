...
When prompted by a `$` we may type commands into it and press _enter_ to run them.
To _run_ a command one must (a) [parse it](https://en.wikipedia.org/wiki/Parsing#Computer_languages), and (b) [interpret it](https://en.wikipedia.org/wiki/List_of_command-line_interpreters) e.g. invoke functions previously defined by the system or user.
We use the wonderful [mvdan-sh](https://github.com/mvdan/sh) to parse commands,
and our own specially written interpreter.
Collectively, the parser and interpreter convert POSIX-like commands into JavaScript.
