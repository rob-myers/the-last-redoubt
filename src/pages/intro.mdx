...
Then our main motivation is _lacklustre NPC behaviour in modern video games_.
Can we clarify the problem?
Can we do better?

Sometimes Game AI is _emergent_.
For example, the brilliant Super Mario 3 consists of mostly independent individual NPC behaviours running in parallel.
But for anything much beyond [Koopa Troopas](https://en.wikipedia.org/wiki/Koopa_Troopa) (fixed patrol route) or [Boo Diddleys](https://en.wikipedia.org/wiki/Boo_(character)) (approach Player when unseen),
the result becomes a mess.
...
Let's summarise the preceding sentences.
...
Importantly, the videos also show a [terminal](https://xtermjs.org/) (a black window with mostly green text).
...
The terminal lies at the heart of our approach.
...
> Try clicking _interact_ above.
>
> You'll see a few lines then repeated messages "__polling for cached query__"
> (if not, click reset <Icon icon="refresh" small inline />).
> Once the repeated messages are seen, click disable <Icon icon="cross-circle" small inline />.
> You'll see a line "__paused session__" and the _interact_ button again.
