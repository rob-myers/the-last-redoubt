...
> Making **Game AI**: the algorithms and data structures used to control [NPCs](https://en.wikipedia.org/wiki/Non-player_character) in video games.

AI stands for Artificial Intelligence and NPC means Non-Player Character, i.e.
every character (enemy or friendly) except for the Player.
NPCs must behave realistically, steadily drawing the Player into the fabrication.
The motivation for this website is _lacklustre NPC behaviour in modern video games_.
Can we do better? Can we understand the problem?

Sometimes Game AI _emerges_ from mostly independent individual NPC behaviours.
But for anything beyond [Koopa Troopas](https://en.wikipedia.org/wiki/Koopa_Troopa) or [Boo Diddleys](https://en.wikipedia.org/wiki/Boo_(character))
(e.g. other human characters),
the result is a mess.
The _combinatorial explosion of possibilities_ inevitably results in
...
The Director and Player interact via the environment, the Player having degrees of freedom within it.
...
> and JavaScript-driven rendering/simulation.
...
We need narratives, characters, and a world they inhabit.
...
The videos also show some kind of _terminal_, but first we summarise the preceding sentences.
...
In our system, the Director _directs_ by running commands in a terminal.
These commands may include scripts and long-running processes.
For example, each terminal runs a script initially called its _profile_.
