...
3. Finally return to the 1st tab and scroll to the top (if necessary).
  ...
  | <small>`source /etc/game-1`</small> | From `/home/PROFILE`, <InlineCode>/etc/util-1</InlineCode> defines game-related functions. Try listing all functions in the console: <InlineCode>declare \| log</InlineCode>. |
  | <small>`await-world`</small> | From `/home/PROFILE`, this command awaits a world with worldKey <InlineCode>/home/WORLD_KEY</InlineCode>. While polling it sends lines, as we saw. |
