...
In fact, the stepwise process is relevant to a wide audience: web developers.
...
Likewise, believable NPC behaviour can be viewed as a kind of Artificial Intelligence.
In particular, we needn't identify AI with Computer Vision or Driverless Vehicles (impressive as they are).

Lacking an abstract approach forces us to embed into a game.
_So what's ours going to be?_
...
The protagonist must escape through ~10 levels of an isolated military base, where every NPC is hostile.
The top-down restricted view coupled with sound-design and general difficultly produce a feeling of dread.
