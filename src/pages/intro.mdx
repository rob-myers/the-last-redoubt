...
To create a game we must _create a world_ i.e. something we can see and move around in.
...
We summarise some of what we've seen.
...
Let's [refer back](#intro--tech-stack "@anchor") to our everyday web dev approach.
Consider the lighting (the lit part of the rooms) which changes as the Player opens/closes/enters doors.
We achieve it using two copies of the [same PNG](/geomorph/g-301--bridge.png "@new-tab"), one atop the other.
...
Rather than relying on Player-interaction with mostly independent parallel systems,
we'll intentionally modify/monitor the Player's experience.
Although a persistent world must exist,
it is the Player's experience we care about, not an ongoing simulation.

> Examples of a modify/monitor approach:
> - Spawn nearby enemies if the Player walks through 3 doors.
> - Build tension by only showing enemies at edge of sight-polygon.
> - Tempt Player towards particular area, then ambush them.

The previous section saw some good progress towards fabricating a world.
But how to shape/control/direct it?

The [videos above](#intro-first-videos "@anchor") show a [_terminal_](https://xtermjs.org/ "@new-tab") i.e. the black window with mostly green text.
