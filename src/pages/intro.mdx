...
That is, we're talking about [_video games_](https://en.wikipedia.org/wiki/Video_game)
and their so-called Artificial Intelligence.
...
Reading Game AI tutorials, one gets the impression that believable behaviour just _emerges_ from the right ingredients.
Now, given NPCs with their own desires and strategies, we can certainly run them in parallel with the Player.
...
We're writing about Game AI and we're gonna make a game.
Then it is natural to apply web technologies, building the game right here, on this site.
But which web technologies?

> We'll use the _everyday practices_ of a Web Developer.
>
> This rules out game-centric technologies like Flash, HTMLCanvas frameworks, CSS 3D transforms, and WebGL.

Importantly, it also makes our approach relevant to a wide audience.


### Building a world

We need narratives and characters and a world they inhabit.
Our setting is _The Last Redoubt_: a gigantic pyramid containing the last of humanity, as described in [The Night Land](https://en.wikipedia.org/wiki/The_Night_Land).
We mention two characters: Andros (the protagonist) and Cartesius (his Preceptor);
these names come from [The Night Land, A Story Retold](https://en.wikipedia.org/wiki/The_Night_Land#Pastiche,_homages_and_sequels).
Our chosen overarching narrative will be familiar: Andros must defend the inhabitants of the pyramid from invading evil forces.
But what about the pyramid?
How should we build The Last Redoubt?

> Thankfully we've already done the groundwork, as shown in the videos below.
...
<aside>
  ...
  These wonderful assets provide a foundation upon which our game is built.
...
Let us turn our attention to the [terminal](https://xtermjs.org/),
the black pane with mostly green text.
...
The Director _directs_ by running commands in a terminal, including scripts and long-running processes. Each terminal runs a script initially, called its _profile_.
