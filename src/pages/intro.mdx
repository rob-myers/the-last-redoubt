...
We've also made some illustrative videos.
...
(supporting systems sans unification).
...
Our approach is relevant to a wide audience: web developers.

We don't believe Game AI can be studied abstractly.
It has very little relationship with Artificial Intelligence in the academic sense.
On the other hand, the [Turing Test](https://en.wikipedia.org/wiki/Computing_Machinery_and_Intelligence) was originally a gender-bending party game.
Convincing NPC behaviour is certainly some kind of Artificial Intelligence.
We needn't identify AI with Computer Vision or Driverless Vehicles (impressive as they are).

To make things meaningful we need the structure of a game.
So what's ours going to be?

> **Setting**.
> A gigantic pyramid housing the last of humanity on Earth, millions of years after the Sun has died. This structure is known as [The Last Redoubt](https://en.wikipedia.org/wiki/The_Night_Land).

> **Story overview**.
> Andros must defend the pyramid against invaders: mental effluents and unnatural incarnations who've broken through the Air Clog.

> **Influences**.
> 1. [Teleglitch](https://en.wikipedia.org/wiki/Teleglitch "@new-tab").
> An excellent top-down survival-horror roguelike, based around difficult combat with dwindling resources.
> 1. [Fallout 1 & 2](https://en.wikipedia.org/wiki/Fallout_(video_game) "@new-tab").
> Brilliantly written and beautifully rendered oblique-projected turn-based combat-driven RPGs.
> 1. [GTA 1 & 2](https://en.wikipedia.org/wiki/Grand_Theft_Auto_(video_game)).
> Brilliant top-down mission-based open-world games.
