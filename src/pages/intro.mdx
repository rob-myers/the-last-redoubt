...
import { ImageCarousel } from "components/page/Carousel";
...
__TODO__ mention Starship Geomorphs as source here
...
The Player moves through rooms by opening nearby doors, and navigating the navmesh.
The Player is _situated_, with limited visual awareness determined by their position.
...
<ImageCarousel
  ...
  imgStyles={{ marginTop: '-32px', height: 'calc(100% + 32px)' }}  // Crop
/>

<!-- TODO intro Tabs below -->

Creating video games is _hard_. Many systems must collectively operate as one:
assets, animation, level design, pathfinding, scripting, controls, UI, dialogue etc.
Often games unify these systems by following the approach of Films.
That is, the story and characters (including the Player) sit on the podium,
with everything else playing a supporting role.
Even if the 

But to put Game AI on the podium, we need a different role model:
