...
Then our world is in-keeping with an [everyday web dev approach](#intro--tech-stack "@anchor").
...
Rather than rely upon the Player interacting with independent parallel systems,
we'll intentionally modify the Player's experience.
Examples include:
- Spawn enemies if the Player walks through 3 doors.
- Build tension by only showing enemies at fringes of sight-polygon.
- Tempt Player to particular area, then ambush them.

We've made good progress towards building a world,
...
These include scripts and long-running processes.
