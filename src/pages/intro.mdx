...
These all-important Non-Player Characters must move in a realistic manner,
drawing the player into the fabrication -- and keeping them there.

Many hope for emergent behaviour.
Given NPCs which can navigate the game world, equipped with their own desires and stratagems,
one can certainly "run them" in tandem with the Player.
Sadly, a _combinatorial explosion of possibilities_ arises,
resulting in unrealistic, unpredictable and fundamentally hard-to-debug behaviour.

Focus on Player Experience yields a much more successful approach.
There's the Player and the _Environment_, the latter controlled by the _Director_.
Actually, a simulated world of NPCs is only needed inasmuch as the Player notices.
This approach is definitely the way to go.
But in order to develop meaningful Game AI, it means we'll need to create a game.
