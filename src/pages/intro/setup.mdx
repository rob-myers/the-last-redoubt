...
Using SVGs permits us to specify rectangles, polygons and points.
We can tag them with space-separated strings by naming them (again encoded as an SVGTitleElement).
Boxy SVG allows us to create/edit/extend this geometric data with ease.
But it remains to explain how we'll actually use these SVG files.

### 2. Asset Pipelines

If you [try running the video game](/#demo) and look in the Network tab (also possible [on a Mobile](/assets/pics/demo-network-devtools-mobile.png "@new-tab") via [Eruda](https://github.com/liriliri/eruda)) you'll see geomorph PNGs and geomorph [JSONs](https://en.wikipedia.org/wiki/JSON). 
These are the geomorph assets seen by the end-user i.e. `{geomorph}.png` and `{geomorph}.json`.

<Carousel
  height={500}
  items={[
    { src: '/assets/pics/demo-network-devtools.png', label: 'Demo Network requests' },
  ]}
/>

But given our 66 `{symbol}.png` and 66 `{symbol}.svg`,
how do we create the particular files `g-301-bridge.png` and `g-301-bridge.json`?
...
<Carousel
  ...
  items={[
    { src: '/assets/pics/demo-network-bridge-png.png', label: 'Bridge Geomorph PNG' },
    { src: '/assets/pics/demo-network-bridge-json.png', label: 'Bridge Geomorph JSON' },
  ]}
/>
