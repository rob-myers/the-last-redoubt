...
But given our 66 symbol PNGs and 66 symbol SVGs,
...
Here's an overview of our 1st pipeline (with a running example).
...
1. From the root of our [code repo](https://github.com/rob-myers/the-last-redoubt/), run the command:
  > <InlineCode>yarn rename-pngs symbol media/Symbols/Staterooms media/symbol-staterooms</InlineCode>
  
  which renames and trims the PNGs and stores a [manifest.json](https://github.com/rob-myers/the-last-redoubt/blob/staging/media/symbol-staterooms/manifest.json "@new-tab"). <SideNote>[rename-pngs.js](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/scripts/rename-pngs.js "@new-tab") also runs `convert -fuzz 1% -trim -colors 32` (ImageMagick)</SideNote>
  We can skip this step if we've encountered a stateroom (bedroom) symbol before.
1. Copy the desired PNG to `media/symbols-png-staging` and manually cut out doors & letters.
1. Run the command:
  > <InlineCode>yarn simplify-pngs media/symbols-png-staging</InlineCode>

  which runs both `convert -fuzz 1% -trim -colors 32` (ImageMagick) and [`optipng`](https://optipng.sourceforge.net/ "@new-tab").
1. Finally start creating the symbol in Boxy SVG:
   - Copy the resulting file to `static/assets/symbol/stateroom--036--2x4.png`.
   - Create `static/assets/symbol/stateroom--036--2x4.svg` in Boxy SVG.
   - Import `static/assets/symbol/stateroom--036--2x4.png` as image in folder named _background_.
