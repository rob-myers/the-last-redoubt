...
Technically a text editor would do, but there's a much better way: [Boxy SVG](https://boxy-svg.com/).
...
We use certain top-level folders e.g. _obstacles_, _singles_ and _walls_.
Groups are locked or hidden by applying styles i.e. _pointer-events:none_ or _visibility: hidden_ respectively.
Descendants can be unlocked/hidden by applying a converse style. 
The underlying SVG document is visible and can be directly edited.

Using SVGs permits us to specify rectangles, polygons and points.
We can tag them with space-separated strings by naming them, which corresponds to adding an SVGTitleElement in the underlying SVG.
Boxy SVG allows us to edit/extend this geometric data with ease.
But how do we actually use these SVGs?
