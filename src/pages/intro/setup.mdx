...
We may tag them with space-separated strings by naming them (a SVGTitleElement in the underlying SVG).
...
But given 66 symbol PNGs and 66 symbol SVGs,
how to actually create these two particular files?
...
Our first pipeline _creates assets_ - it outputs a single `{symbol}.PNG` and a single `{symbol}.SVG`.
We pick a symbol, find the respective PNG, clean it up a bit, and finally create a respective SVG using Boxy SVG.
...
1. Copy the desired PNG to `media/symbols-png-staging` and manually cut out doors & letters (see 3rd slide above).
1. Run the command:
