...
To apply this CLI to Game AI we define various functions:
...
- _cheerio.js_ e.g. extract polygons from metadata in our Boxy-SVG-authored files.
- _dom.js_ e.g. [SVG path](https://www.w3.org/TR/SVG11/paths.html#DAttribute) to polygon; HTMLCanvasElement utilities.
- _geom.js_ e.g. triangulation and intersection of 2d geometry.
- _geomorph.js_ e.g. create a geomorph JSON given transformed symbols.
- _npc.js_ e.g. render spritesheet; npc collision prediction.
- _triangle.js_ is a wrapper around [triangle-wasm](https://www.npmjs.com/package/triangle-wasm), itself a wrapper around [Triangle](https://www.cs.cmu.edu/~quake/triangle.html).
