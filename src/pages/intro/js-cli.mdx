...
- <InlineCode>foo &amp;&amp; bar</InlineCode> run "foo" and if exit code 0 run "bar"
- <InlineCode>foo || bar</InlineCode> run "foo" and if exit code 1 run "bar"
...
- <InlineCode>${'{'}foo:-bar{'}'}</InlineCode> like $foo but if empty output literal "bar"
...
We provide useful shell functions "out of the box".
Here are a few:

```bash
# e.g. `call '() => 42'` outputs the number `42`
call () {
  run '(ctxt) {
...
# e.g. `range 5` outputs the array `[0, 1, 2, 3, 4]`
range () {
  call '({args}) =>
...
# e.g. `range 5 | map '() => x ** 2'` outputs `[0, 1, 4, 9, 16]`
map () {
  run '(ctxt) {
...
# e.g. `range 5 | keys` outputs `['0', '1', '2', '3', '4']`
keys () {
  map Object.keys
}
```
