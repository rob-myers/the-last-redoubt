...
The _combinatorial explosion of possibilities_ produces hard-to-debug behaviour.

Focusing on the Player's experience is a better idea.
Everything besides the Player is called the _environment_.
It is controlled by the _Director_, which is just a fancy name we've given ourselves.
...
<Image
  src="/assets/pics/demo-pic-1.png"
  title="A sneak peek"
...
So our approach uses [everyday web dev tech](#index--2.-underlying-tech) as promised.
...
We call this ongoing mutation of the player's experience _directing_.

</aside>

> But technically-speaking, how can we direct?

The [videos above](#intro-first-videos) show a [_terminal_](https://xtermjs.org/ "@new-tab") (black window with mostly green text).
...
> **we direct by running commands in one or more terminals**.
>
...
| `awaitWorld` | From `/home/PROFILE`, this command awaits a world whose identifier is the string at <InlineCode>/home/WORLD_KEY</InlineCode>. While polling it sends lines, as we saw. |
...
The protagonist must escape a military base.
...
Then a level or scenario (part of a level) should be kept in mind whenever we develop Game AI.
...
Articles up next:
- **Setup** -- _how we done built what we done built_.
- **JS CLI** -- _our in-browser JavaScript Command Line Interface_.
- **AI CLI** -- _applying the JS CLI to Game AI_.
