...
We believe video games have great potential e.g. they provide a chance to better understand ourselves.
...
These particular games avoid simplistic character behaviour by employing a **tight narrative**, combined with mini-games or constrained scenarios.
...
<Carousel
  ...
  height={550}
  fullHeight={900}
  items={[{ video: 'first-peek-test-2', label: 'First peek' }]}
/>

Both meaningless violence and bad storytelling may actually be _side-effects_ of simplistic character behaviour. If the behaviour was more sophisticated it could help tell the story, and the game-mechanic needn't purely revolve around violence. But how to develop more sophisticated character behaviour?
...
{/* [NPC](https://en.wikipedia.org/wiki/Non-player_character) */}

1. Focus on creating and combining character behaviours without trying to create a video game.
...
1. Use openly available assets:
    - Robert Pearce's [Starship Geomorphs](http://travellerrpgblog.blogspot.com/2020/07/starship-geomorphs-20.html "@new-tab").
...
Then we'll be focussing on the problem directly by creating and combining many behaviours for their own sake (usually Game AI is beholden to its parent video game).
...
<Aside title="of-general-interest">
  We tackle _general interest_ in a few ways.

  Firstly, our implementation uses standard web development techniques.
  We use [CSS](https://en.wikipedia.org/wiki/CSS) and the [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API) for rendering, as opposed to inner loops based on [`requestAnimationFrame`](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame).
  Then we are applying techniques used by legions of web developers on a daily basis.
  Secondly, our approach to scripting is of a general nature.
  We provide an interactive shell which interprets POSIX syntax as JavaScript and executes it. This is possible by virtue of the wonderful shell parser [mvdan-sh](https://github.com/mvdan/sh/tree/master/_js), plus recursively defined [async generators](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AsyncGenerator).
  The resulting [CLI](https://en.wikipedia.org/wiki/Command-line_interface) (Command Line Interface) gives our website its name.
  Finally, we'll stick to gameplay relevant to a huge raft of video games.
</Aside>

So much for generality... but will it actually be entertaining?
We'll program NPCs, and programming requires patience.
Entertainment should arise by building a little world and interacting with it.
As the complexity of these fabrications increases, so will our satisfaction.
...
🚧 Use web technology the way it is supposed to be used.
