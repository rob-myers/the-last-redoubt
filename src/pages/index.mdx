...
Both meaningless violence and bad storytelling may actually be _side-effects_ of simplistic character behaviour. If the behaviour was more sophisticated it could help tell the story,
...
1. Avoid an abstract approach by using the [Traveller](https://en.wikipedia.org/wiki/Traveller_(role-playing_game)) universe as a backdrop.<SideNote width={300}>The concept of Starship Geomorph comes from Traveller.</SideNote>
...
So, we are going to create and combine character behaviours.
But what actually _is_ an character behaviour?
To answer this question we need some underlying concepts:
Player, NPC, and World.

<Def title="world">

  #### Definition
  
  A _World_ is an instance of the React function component [`World`](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab").
  It has props:
  - `disabled`: boolean to pause the World.
  - `worldKey`: string identifying the World.
  - `gms`: list of objects `{gmKey, transform}` i.e. Starship Geomorphs and where to put them.

</Def>

At present, we support 5 Starship Geomorphs<SideNote>Robert Pearce provided hundreds of Starship Geomorphs</SideNote> constructed from 75 of Robert Pearce's [Starship Symbols](http://travellerrpgblog.blogspot.com/2020/08/starship-symbols-book.html "@new-tab")<SideNote>Available in PNG format on [Eric Smith's site](http://ericbsmith.no-ip.org/geomorphs/ "@new-tab").</SideNote>
...
These geomorphs should be transformed so their hull doors either coincide with another hull door (or are disconnected). For example, here are two `301`s and two `101`s.
...
<Tabs
  ...
  tabs={[
    [
      { type: 'component', filepath: 'vanilla-world-1' },
    ],
  ]}
/>
