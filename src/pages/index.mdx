...
Answering this question requires some underlying concepts:
...
<Def title="world">
  ...
  It has props:
  - `disabled` - a boolean to pause the World.
  - `worldKey` - a string identifying the World.
  - `gms` - an array of objects `{gmKey, transform}` i.e. geomorphs and where to put them.
...
For example, here are two different ways of combining `101` and `301`.
...
<Tabs
  ...
  tabs={[
    [
      { type: 'component', filepath: 'map-only-1' },
      { type: 'component', filepath: 'map-only-2' },
    ],
  ]}
/>
