...
By _browser-based CLI_ we mean an interactive Command Line Interface running in the browser.
...
Supported syntax includes pipelines, shell functions and background processes.
...
<aside>
  ...
  In the above video we (a) list available shell functions, (b) inspect some implementations, (c) chain them together using pipelines.
  
  > But how is the shell syntax interpreted as JavaScript?

  Well, `call` invokes a supplied JavaScript function
  and `run` iterates a supplied JavaScript async iterable function.
  In fact `call`, `range`, `split` and `map` are all defined via the builtin `run`.
