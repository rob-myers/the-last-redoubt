...
import { ansiColor } from "projects/sh/util";
...
They have become synonymous with meaningless violence, bad storytelling and **simplistic character behaviour** ([NPC](https://en.wikipedia.org/wiki/Non-player_character "@new-tab") as a [meme](https://en.wikipedia.org/wiki/NPC_(meme)  "@new-tab")).
...
1. Develop differently:
    - Canonical visual representations via HTML, CSS, [WAAPI](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Web_Animations_API_Concepts).
    - Canonical in-browser scripting via [POSIX](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/V3_chap02.html#tag_18) syntax interpreted as JavaScript.
...
1. Avoid an abstract approach by using the [Traveller universe](https://en.wikipedia.org/wiki/Traveller_(role-playing_game)) as a backdrop.<SideNote>The concept of Starship Geomorph comes from Traveller.</SideNote>

Then we intend to approach the problem directly,
by creating and combining behaviours for their own sake.
But this is a difficult thing to do: we risk creating something neither of **general interest** nor with much **entertainment value**.
...
relevant to a wide audience: web developers.
...
- Our approach to in-browser scripting is generic: an interactive shell interpreting POSIX syntax as JavaScript.<SideNote>Achieved via the wonderful shell parser [mvdan-sh](https://github.com/mvdan/sh/tree/master/_js), interpreted recursively as [async generators](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AsyncGenerator).</SideNote>
...
- We consider gameplay relevant to a huge number of video games, viewed from a top-down perspective.

But what about _entertainment value_?
If we're going to make video games more meaningful,
they need to be more entertaining too.
Additional depth must be integrated into the experience.
This can be subtle e.g. deterministic behaviour may be desirable to avoid frustration.
A Player-centric approach is required,
as opposed to complex simulations possibly never witnessed by the Player.
Finally, we must take care that our desire for generality does not produce a dry abstract approach.
...
For behaviour to arise we need a _World_ and denizens therein.
Defining a World amounts to choosing [Starship Geomorphs](http://travellerrpgblog.blogspot.com/2020/07/starship-geomorphs-20.html "@new-tab") and connecting them together.
At present, we only support 5 geomorphs (Robert Pearce provided hundreds) constructed from about 80 of [Starship Symbols](http://travellerrpgblog.blogspot.com/2020/08/starship-symbols-book.html "@new-tab")<SideNote>Available in PNG format on [Eric Smith's site](http://ericbsmith.no-ip.org/geomorphs/ "@new-tab").</SideNote> (he provided thousands).
...
The above pan/zoomable maps are actually _Worlds_ i.e. [World.js](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab") component instances.
For example, the first map amounts to the following component instantiation:
...
The `worldKey` provides a unique identifier into a cache where the World's API is stored.
`gms` specifies the geomorphs and their layout, as outlined above.
But why do these worlds look like maps?
Because there is no _Player_ and consequently no _FOV_ (Field Of View).<SideNote>We already saw the Player-sensitive FOV in the [video earlier](#first-peek-video).</SideNote>

Then the next step towards behaviour is a World with a Player.
This brings us to a very important ingredient: the `<Terminal>`.


<Tabs
  id="tabs@world-with-tty-1"
  initEnabled={false}
  height={[600, 600]}
  persistLayout
  tabs={[
    [
      { type: 'component', class: 'World', filepath: 'world@world-with-tty-1', props: {
        worldKey: 'world@world-with-tty-1',
    ...
    [
      { type: 'terminal', filepath: 'tty@world-with-tty-1', env: {
        WORLD_KEY: 'world@world-with-tty-1',
        PROFILE:
`source /etc/util-1
source /etc/game-1
...
NOECHO=1 echo "${ansiColor.Reverse}So far we've loaded shell functions (declare -F) and connected to the World.${ansiColor.ReverseReset}"
NOECHO=1 choice 'tap to [continue](-)'

spawn foo '{x:-89.8,y:1075.7}'
npc set-player foo
view 1000 '{x:-89.8,y:1075.7}' 2

NOECHO=1 echo "${ansiColor.Reverse}We've spawned an npc, set them as the player, and finally pan-zoomed them into view.${ansiColor.Reset}"
NOECHO=1 choice 'tap to [continue](-)'
...
NOECHO=1 echo "${ansiColor.Reverse}We hid the map overlay, setup movement (tap navmesh) and actions (tap doors or icons) via background processes.${ansiColor.Reset}"
`
      }},
    ],
  ]}
