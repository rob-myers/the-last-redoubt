...
Given a textual description of a JavaScript function and its arguments, it creates the JavaScript function and its arguments, and finally invokes it.
Less obviously, the shell builtin `run` iterates a supplied JavaScript AsyncGenerator.
The shell functions `call`, `range`, `split` and `map` are all defined in terms of `run`.
