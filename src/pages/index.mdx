...
We want to make video games more meaningful. That is the objective of this website. Video games suffer from an underlying problem: **simplistic character behaviour** (NPC as a [meme](https://en.wikipedia.org/wiki/NPC_(meme))). Exceptional games have circumvented this.
...
These games avoid simplistic behaviour by employing a **tight narrative**, combined with exploration, mini-games and constrained scenarios.
Nevertheless, superficial character behaviour is the norm for video games.
...
As far as more meaningful video games are concerned,
we believe character behaviour is the main blocker. 
For example, simplistic behaviour often yields meaningless violence (violent behaviour is easier to implement).
For example, the story may seem disjoint from gameplay because in-game behaviour does not help to tell it.
Rather than avoiding the problem by tightening the narrative,
we'd prefer to develop more sophisticated behaviour, e.g.
...
<Aside title="clarify-sophisticated-behaviour">
  ...
  e.g. an NPC with two attacks has a choice.
  But these behaviours are rarely composable.
  For example, a polite NPC might let the Player through a door first (choice),
  and notice something while waiting (curiosity),
  which they later recollect (time sensitive).

  We are particularly interested in combining behaviours.
</Aside>

But how are we going to create more sophisticated behaviour?
We have a few core ideas.<SideNote>Viewed from an aerial standpoint.</SideNote>
