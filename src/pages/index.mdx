...
- Our [CLI](https://en.wikipedia.org/wiki/Command-line_interface) is extended with generic [NPC](https://en.wikipedia.org/wiki/Non-player_character) shell functions<SideNote>CLI stands for _Command Line Interface_, and NPC stands for _Non-Player Character_.</SideNote><SideNote>These shell functions give our website its name.</SideNote> e.g. `click`, `spawn`, `look`, `nav`, `walk`.
