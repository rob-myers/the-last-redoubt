...
- Our approach to in-browser scripting is of general interest.
...
- We consider gameplay relevant to a huge number of video games, presented from a top-down perspective.

But what about _entertainment value_?
To make video games more meaningful, we must make them more entertaining too.
This is subtle e.g. deterministic behaviour may be desirable to avoid frustrating players during a combat scenario.
A Player-centric approach is required, as opposed to building simulations possibly never witnessed by the Player.
We must also avoid a dry abstract approach.
...
At present, we only support 5 geomorphs (Robert Pearce provided hundreds) constructed from about 80 of his [Starship Symbols](http://travellerrpgblog.blogspot.com/2020/08/starship-symbols-book.html "@new-tab")<SideNote>Available in PNG format on [Eric Smith's site](http://ericbsmith.no-ip.org/geomorphs/ "@new-tab").</SideNote>.
...
The above pan/zoom-able maps are actually _Worlds_ i.e. [World.js](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab") component instances.
