...
In one approach the Game defines an ongoing simulation involving individual NPCs.
...
Concerning video games, we'll try not to comment on the current state of affairs.
...
Let's clarify the second distinguishing point above.
...
Concerning the above video, every shell session begins by running the script at `/home/PROFILE`.
In this particular session,
the latter resolves to the following [string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String):
...
Our shell builtin `source`<SideNote width={300}>It behaves like the [Bash builtin](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html#index-source), a synonym for a [Bourne shell builtin](https://www.gnu.org/software/bash/manual/html_node/Bourne-Shell-Builtins.html#index-_002e).</SideNote> runs the script at `/etc/util-1`,
a string defining a bunch of shell functions e.g. `range`.
In the above video we go through this profile, explaining the various commands and their connection to the `World`.
...
Let's clarify our third distinguishing point.
...
A typical website amounts to a deep [DOM-tree](https://en.wikipedia.org/wiki/Document_Object_Model) initially induced by HTML, brought to life _visually_ via CSS and _interactively_ via JavaScript.<SideNote>Technically, JavaScript permits arbitrary DOM mutation.</SideNote>
The browser interprets CSS very efficiently.
Moreover CSS can avoid re-inventing the wheel through heavy-duty JavaScript.<SideNote>Although HTMLCanvas is performant via [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API), the latter is a low-level API.</SideNote>
...
🚧 browser devtool video

To write about NPC behaviour we'll need to create many flexible interactive demos.
This means we need to write software.

- An in-browser CLI permits writing, running and managing this software _explicitly_.

- CSS-based rendering permits performant rendering with rich debugging (browser devtool).

Both technologies are used heavily in a vast number of disparate domains,
e.g. development environments, infrastructure as a service, personal websites, rich web applications, and [desktop applications](https://www.electronjs.org/) like Visual Studio Code.
Aligning ourselves to these technologies will help us to avoid creating yet another interface island.
