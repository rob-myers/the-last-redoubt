...
A Player-centric approach is required, as opposed to building simulations which may never be witnessed by the Player.
We must also avoid a dry abstract approach:
we'll lean on [Traveller RPG](https://en.wikipedia.org/wiki/Traveller_(role-playing_game)) for context and background.
...
The World is an instance of a React function component, as is each NPC.
But how are they related?
Let's look at the internals of three components: [World.jsx](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab"), [NPCs.jsx](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/NPCs.jsx "@new-tab") and [NPC.jsx](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/NPC.jsx "@new-tab").
To avoid confusion we'll simplify them considerably.


```jsx
/** internals and return value simplified */
export default function World(props) {
  const state = useStateRef(() => { /** `api` */ });

  return state.gmGraph.ready ? (
    <CssPanZoom onLoad={api => state.panZoom = api}>
      <Geomorphs api={state} onLoad={api => state.geomorphs = api} />
      <DebugWorld api={state} onLoad={api => state.debug = api} />
      <Decor api={state} onLoad={api => state.decor = api} />
      <NPCs api={state} onLoad={api => state.npcs = api} />
      <Doors api={state} onLoad={api => state.doors = api} />
      <FOV api={state} onLoad={api => state.fov = api} />
    </CssPanZoom>
  ) : null;
}
```
...
export default function NPCs(props) {
  const state = useStateRef(() => { /** `api.npcs` */ });

  return (
    <div className="npcs" ref={state.rootRef}>
...
export default function NPC({ api, npcKey }) {
  ...
  return (
    <div
      ...
      className={`npc ${npc.anim.spriteSheet} ...`}
    >
      <div className={`body webp ${npc.key}`} />
