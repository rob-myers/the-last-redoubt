...
Concerning video games, we'll try to remain silent concerning the current state of affairs.
...
<Aside title="screenshots">
  ...
  The other images show the `Tabs`, `World` and `Terminal` in other states.<SideNote width={500}>Some of the commands shown in the terminals are out-of-date. In particular we now `npc config debug` to toggle debug mode, whereas the images use `DEBUG=1; npc config`</SideNote>

</Aside>

Let's clarify the distinguishing points above.
...
Every shell session begins by running the script at `/home/PROFILE`.
In the session above it is the string <InlineCode>source /etc/util-1</InlineCode> plus a trailing newline.
Our shell builtin `source`<SideNote width={300}>See the [bash builtin](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html#index-source), itself a synonym for a [Bourne shell builtin](https://www.gnu.org/software/bash/manual/html_node/Bourne-Shell-Builtins.html#index-_002e).</SideNote> runs the script at `/etc/util-1`,
...
A typical website amounts to a deep oft-mutated [DOM-tree](https://en.wikipedia.org/wiki/Document_Object_Model) initially induced by HTML, brought to life _visually_ via CSS<SideNote width={220}>CSS also supports some interactivity e.g. [:hover](https://developer.mozilla.org/en-US/docs/Web/CSS/:hover)</SideNote> and _interactively_ using JavaScript<SideNote>Technically, JavaScript permits arbitrary DOM mutation.</SideNote>.
The browser interprets CSS very efficiently.
In particular, it avoids re-inventing the wheel through heavy-duty JavaScript.<SideNote>Although HTMLCanvas is performant via [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API), the latter is a low-level API.</SideNote>
...
🚧 Illustrate CSS via devtool video

🚧 Two canonical UIs: POSIX-like CLI + Browser DevTool
