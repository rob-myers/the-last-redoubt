...
This is **a difficult thing to do**: we risk creating something neither of _general interest_ nor with much _entertainment value_.
...
<Carousel
  ...
  items={[
    { src: 'npcs-1.png', webp: 'npcs-1.webp', label: 'A World (left) and a Terminal (right)' },
    { src: 'npcs-2.png', webp: 'npcs-2.webp', label: 'Each World provides a birdsye view.' },
    { src: 'npcs-3.png', webp: 'npcs-3.webp', label: 'Each World is driven by one or more Terminals.' },
  ]}
/>
...
So, we are going to create and combine NPC behaviours.
But what actually _is_ an NPC behaviour?
To answer this question we first need some underlying concepts:
Player, NPC, and World.

<aside title="definition-world">
  **Definition**: A _World_ is an instance of the React component [`World`](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab"). It has props:
  - `disabled`: a boolean used to pause the World.
  - `worldKey`: a string identifying the World.
  - `gms`: a list of objects `{gmKey, transform}` i.e. Starship Geomorphs and where to put them.

  We currently support 5 Starship Geomorphs:<SideNote>Robert Pearce provided hundreds...</SideNote>
...
Then a World amounts to a bunch of Geomorphs connected together.
For example:
