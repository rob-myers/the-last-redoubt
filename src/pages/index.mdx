...
1. Use the [Traveller universe](https://en.wikipedia.org/wiki/Traveller_(role-playing_game)) as backdrop <SideNote>The concept of _Starship Geomorph_ comes from Traveller.</SideNote>, plus a narrative container for our behaviours: "The Experiment".
...
A _Player-centric approach_ can avoid these pitfalls.
But there is also another important problem, insofar as entertainment is concerned.
Although we don't want to create a video game,
we also don't want an abstract collection of behaviours.
We provide context on two levels: the setting (Traveller universe) and a _narrative container_ (joining the behaviours together, somewhat vaguely).
We call the latter "The Experiment".
...
We're finally ready to create an NPC behaviour.<SideNote>Recall NPC stands for Non-Player Character.</SideNote>
We at least need a World, the Player and one additional character.
