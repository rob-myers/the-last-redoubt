...
Then we'll approach the problem directly by creating and combining many behaviours for their own sake (Game AI is usually beholden to its parent video game).
...
- Our implementation uses standard web development techniques e.g.
[CSS](https://en.wikipedia.org/wiki/CSS) and the [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API) for rendering, as opposed to inner loops invoking [`requestAnimationFrame`](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame).
...
So much for general interest, but will it actually be _entertaining_?
