...
- Our CLI ([Command Line Interface](https://en.wikipedia.org/wiki/Command-line_interface)) is extended with _generic_ NPC ([Non-Player Character](https://en.wikipedia.org/wiki/Non-player_character)) related shell functions<SideNote>These shell functions give our website its name.</SideNote> e.g. `click`, `spawn`, `look`, `nav`, `walk`.
