...
At present, we support 5 geomorphs (Robert Pearce provided [hundreds](http://travellerrpgblog.blogspot.com/2020/07/starship-geomorphs-20.html "@new-tab")) constructed from about 80 of Robert Pearce's [Starship Symbols](http://travellerrpgblog.blogspot.com/2020/08/starship-symbols-book.html "@new-tab")<SideNote>Available in PNG format on [Eric Smith's site](http://ericbsmith.no-ip.org/geomorphs/ "@new-tab").</SideNote>.
...
The Worlds above lack NPCs and in particular the Player.
Without the Player there are no _sensory organs_ and `<World>` reduces to a pan/zoomable map.
...
<Def title="npc-and-player">
  ...
  The _Player_ is an _NPC_ whose prop `npcKey` equals `api.npcs.playerKey`.
...
{/* 🚧 cleanup how api.npcs.playerKey is used */}
