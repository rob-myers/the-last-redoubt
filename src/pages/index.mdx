...
Before discussing the World's internal state _a.k.a_ external `api`,
we first define _NPC_ and _Player_.
...
<Def title="npc">
  ...
  instantiated by [`<NPCs>`](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/NPCs.jsx "@new-tab").
  
  Its props are `api` and `npcKey` (a string).
  Its internal state is stored at `api.npcs.npc[npcKey]` and
  created using [`create-npc.js`](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/create-npc.js "@new-tab").
