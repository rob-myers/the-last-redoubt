...
Nevertheless, simplistic character behaviour is the norm for video games.
...
> Following a ghost should attract attention (behaviour can be _combined_).

> NPCs can do things in multiple ways (_nondeterministic_ behaviour).

> NPCs remember and forget (behaviour informed by _knowledge_).

But how? We have a few core ideas.
...
Thus we provide context on two levels: the setting (Traveller universe) and also a _narrative container_ (joining the behaviours together).
The latter is called "The Experiment", which has at least two interpretations...
