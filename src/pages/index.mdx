...
Let's see it action.
The Terminal's _profile_ explains what it is doing,
and prompts the user to "tap to continue".
...
NOECHO=1 echo "${ansi.Reverse}${ansi.Bold}Finally we have faded the map and also set up the Player's controls."
NOECHO=1 echo
...
<Aside title="tips-and-tricks">
  ...
  <Carousel
    ...
    mobileHeight={400}
    fullHeight={900}
    items={[{ video: 'first-peek', label: 'Doing all the things' }]}
  />
