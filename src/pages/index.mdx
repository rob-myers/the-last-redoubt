...
The shell function `call` provides a relatively simple example.
...
The shell functions `call`, `map`, `range` and `split` are all defined in terms of `run`.
...
A typical website consists of a deeply nested DOM-tree<SideNote>Initially induced by HTML.</SideNote>, brought to life via CSS (visuals)<SideNote width={200}>CSS supports some interactivity e.g. [:hover](https://developer.mozilla.org/en-US/docs/Web/CSS/:hover)</SideNote> and JavaScript (interactivity)<SideNote>Technically JavaScript permits arbitrary DOM mutation.</SideNote>.
By _declaring_ the visual style via CSS, the browser can make many _performance optimisations_ (dependent on the device and viewport).
_Debugging_ is also far richer because one may directly mutate the DOM-tree in a browser devtool.
On the other hand, debugging `<canvas>` amounts to setting breakpoints inside JavaScript code (or writing a special-purpose debugger).
