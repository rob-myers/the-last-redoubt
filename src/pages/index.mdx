...
Recall our second distinguishing point above:
...
Then only one distinguishing point remains to be explained:

> Rich graphics via Starship Geomorphs.

We want to program NPCs. In more detail, we'd like produce a catalogue of interesting behaviours, and study how they can be composed in response to the Player's actions.
However, these behaviours need to be studied at the right level of abstraction.
We are not trying to create middleware.
...
It is usually brought to life via assets representing a specific situation e.g. animations of denizens and suitable scenery.

🚧 Although we definitely need this "spice", we'll try to keep things generic.
Traditional role-playing games provide a natural source of inspiration...
