...
For example we use [CSS](https://en.wikipedia.org/wiki/CSS) and the [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API) for rendering, rather than a non-declarative approach invoking [`requestAnimationFrame`](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame).
...
We provide an interactive shell interpreting POSIX syntax as JavaScript.<SideNote>Interactively parsed using the wonderful [mvdan-sh](https://github.com/mvdan/sh/tree/master/_js).</SideNote>
  This amounts to a Command Line Interface ([CLI](https://en.wikipedia.org/wiki/Command-line_interface)).

- Our NPC-related shell functions combined with the above gives our website its name.
  The latter have generic appeal, e.g. `click`, `spawn`, `look`, `nav` and `walk`, and can be naturally piped:
  ```bash
  # walk the npc "rob" to any clicked navigable point
...
The latter is called "The Experiment" and has at least two interpretations.
...
<Aside title="can-pan-zoom-tabs">
  ...
  and you can drag their tabs around (or maximise them).<SideNote>Thanks to the wonderful tabs manager [flexlayout-react](https://www.npmjs.com/package/flexlayout-react).</SideNote>
...
This brings us to a very important ingredient i.e. the `<Terminal>`.
...
NOECHO=1 choice "So far, this terminal's profile has:
- defined shell functions
- connected to world \\$WORLD_KEY
${ansi.Bold}tap to${ansi.BoldReset} [continue](-)"

spawn rob '{x: -90, y: 1075}'
npc set-player rob
view 1 '{x: -90, y: 1075}' 1.6

NOECHO=1 choice "Next we've spawned an npc \\"rob\\",
set them as the Player and,
brought them into view.
Try pan-zooming & ${ansi.Bold}tap to${ansi.BoldReset} [continue](-)"
...
NOECHO=1 echo "Finally we have faded the map,
and setup the Player's controls:
