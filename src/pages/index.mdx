...
These particular games overcome simplistic character behaviour by employing a **tight narrative**, combined with mini-games or constrained scenarios.
But what about a more direct approach?

{/* 🚧 careful about videos not in any playlist */}
...
<Carousel
  ...
  items={[
    { video: 'first-peek-test-1', label: 'Foo bar baz' },
  ]}
/>


Actually, meaningless violence and bad storytelling may be _side-effects_ of simplistic character behaviour. If the behaviour was more sophisticated it could help tell the story, and the game-mechanic needn't revolve around violence alone. But how can we develop more sophisticated character behaviour?
...
How will the above help us surmount simplistic character behaviour?
To begin with, we'll focus on the problem directly by creating and combining many behaviours.
Importantly, we are creating them for their own sake - and not inside a video game.
...
<Aside title="of-general-interest">
  ...
  We provide an interactive shell which interprets POSIX syntax as JavaScript and executes it. This is possible via the wonderful shell parser [mvdan-sh](https://github.com/mvdan/sh/tree/master/_js) and recursively defined [async generators](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AsyncGenerator).
...
this is relative to being a programmer,
or being interested in how programming actually works.
Let's take a look at the system we've already built...
