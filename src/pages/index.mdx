...
<Def title="world">
  ...
  `gms` (objects `{gmKey, transform}`) i.e. 
  which geomorphs and where to put them.
  Its internal `state` is stored under `worldKey` in an application-wide [QueryClient](https://tanstack.com/query/v4/docs/react/reference/QueryClient "@new-tab"),
