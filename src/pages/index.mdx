...
Finally, we must take care our desire for generality does not produce a dry abstract approach.
...
We've gone the canonical textual route i.e. POSIX.
Syntactically we want the look and feel of an interactive shell.
Semantically we want to manipulate Javascript values and run async generators. 
...
NOECHO=1 echo "${ansi.Reverse}${ansi.Bold}Finally we've hidden the map and setup the Player. Tap the navmesh (to move), nearby doors/icons (to do things), elsewhere (to look).${ansi.Reset}"
`
      }},
    ],
  ]}
