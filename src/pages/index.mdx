...
- [The Last of Us Part II](https://en.wikipedia.org/wiki/The_Last_of_Us_Part_II) <SideNote width={300}>Admittedly very violent, but not in a meaningless way.</SideNote>
...
These maps are actually _Worlds_ i.e. [World.js](https://github.com/rob-myers/the-last-redoubt/blob/staging/src/projects/world/World.jsx "@new-tab") component instances.
...
These worlds look like maps because there is no _Player_.
The Player determines the Field Of View (FOV) e.g. see the [video earlier](#first-peek-video).
...
<Tabs
  ...
  tabs={[
    [
      { type: 'component', class: 'World', filepath: 'world@first-demo', props: {
        worldKey: 'world@first-demo',
    ...
    [
      { type: 'terminal', filepath: 'tty@first-demo', env: {
        WORLD_KEY: 'world@first-demo',
        PROFILE:
`source /etc/util-1
source /etc/game-1
...
NOECHO=1 echo "${ansiColor.Reverse}So far we've loaded shell functions and connected to the World.${ansiColor.ReverseReset}"
...
NOECHO=1 echo "${ansiColor.Reverse}We've hidden the map overlay. We also setup movement (tap navmesh) and actions (tap doors or icons) via background processes.${ansiColor.Reset}"
`
      }},
    ],
  ]}
