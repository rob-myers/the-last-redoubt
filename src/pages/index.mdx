...
whereas `gms` specifies the geomorphs and their layout (as outlined above).
...
NOECHO=1 choice '${ansi.Reverse}${ansi.Bold}So far, this profile has loaded various shell functions and connected to the World.${ansi.Reset} tap to [continue](-)'

spawn bilem '{x:-89.8,y:1075.7}'
npc set-player bilem
view 1000 '{x:-89.8,y:1075.7}' 1.6

NOECHO=1 choice '${ansi.Reverse}${ansi.Bold}Next we have spawned an npc called "bilem", set them as the Player, and finally pan-zoomed them into view.${ansi.Reset} tap to [continue](-)'
